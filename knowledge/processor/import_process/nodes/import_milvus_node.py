    ...
    def _validate_get_inputs(self, state: ImportGraphState):
        ...
        for chunk in chunks:
            ...
            if chunk.get('dense_vector') and chunk.get('sparse_vector'):
                # 补全 schema 中定义但数据缺失的标量字段，否则 insert 报 DataNotMatchException
                for field in _SCALAR_FIELDS:
                    chunk.setdefault(field.field_name, "")
...
def _cli_main() -> None:
    ...
    temp_dir = Path(
        r"D:\PythonProgram\PythonProject\2026.8.22\shopkeeper_brain\knowledge\processor\import_process\import_temp_dir\万用表的使用\hybrid_auto")
