...
def get_beg_m3_embedding_model():
    ...
    try:
        ...
        logger.error("无法导入 pymilvus.model.hybrid，请降级 pymilvus 到 2.3.x")
        return None
    ...
    try:
        ...
        logger.error(f"BGEM3EmbeddingFunction 初始化失败: {e}")
        return None
...
def generate_hybrid_embeddings(embedding_model, embedding_documents: List[str]):
    """
    为文本生成向量嵌入
